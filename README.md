...
Finite memory gives rise to constraints on the depth and breadth of constructed sentences. In other words, the amount of memory for which a rewrite agent has at its disposal at any given time has direct influence on the complexity limit, or the maximum number of nodes that a rewrite agent can keep track of in a constructed sentence.

Phonological patterns and their corresponding words are connected through bidirectional links in memory, such that the phonological pattern and the semantic reperesentation are stored as individual nodes, and their shared bidirectional link allows activation signals to propagate from each node to the other. Semantic representations are also connected to motor patterns that are meant to emulate the corresponding phonological pattern. By sending motor patterns to a verbal-actuator system, a signal is produced and propagated through the social environment- a network of agents that send and receive messages- and is received by neighboring agents as a sequence of phonological information, such that a sequence of output signals from an given agent over a period of time is sent down a set of transmission lines while maintaining the temporal relations between the individual patterns that make up the sequence, which are produced one-by-one at each new time step.
